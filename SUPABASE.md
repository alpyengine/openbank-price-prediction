...
create table public.batches (
  ...
  hit_rate_close integer,                           -- close hit rate % (hits+close) — v7.19.0
  hit_rate_ext   integer,                           -- extended hit rate % (hits+close+exceeded) — v7.19.0: now includes close
...
**Hit-rate ladder** *(v7.19.0)* — each metric includes the previous one; `miss` and
`wrong_way` never count toward any of them:

```
hit_rate       = hit / evaluated
hit_rate_close = (hit + close) / evaluated
hit_rate_ext   = (hit + close + exceeded) / evaluated
```

All 3 are recomputed for every batch by `recalc_hit_rates()` (see §7).
...
7. Recalculates `hit_rate`, `hit_rate_close`, and `hit_rate_ext` for all affected batches *(v7.19.0: added `hit_rate_close`, `hit_rate_ext` now also includes `close`)*
...
| `recalc_hit_rates()` | Recomputes `hit_rate` / `hit_rate_close` / `hit_rate_ext` for all evaluated batches *(v7.19.0)*. |
