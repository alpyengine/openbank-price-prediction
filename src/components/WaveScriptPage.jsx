 ...
 *
 * Colour by chart order: 1st=red, 2nd=blue, 3rd=green, then orange/purple cycle.
...
// One wave per batch projection. Colour follows chronological order.
...
`
  ...
  const pushes = waves.map((w, i) => {
    const label = `${w.ticker}  (${w.company})  base ${new Date(w.baseDate).toLocaleDateString('es-ES')}`
    return `    // [${i}] ${label}
...
export default function WaveScriptPage({ batches = [] }) {
  ...
  const stats = useMemo(() => {
    const tickers = new Set(waves.map(w => w.ticker))
