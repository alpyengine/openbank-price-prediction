      ...
      {reportType === 'candidates' && (
        <>
          ...
          <Card className="p-5">
            ...
            <div className="text-[11px] text-muted-foreground mt-4 pt-3 border-t border-dashed border-border">
              Coinciden <b className="text-foreground">{filteredCandidates.length} instancias</b> de{' '}
              <b className="text-foreground">{aggregatedCandidates.length} tickers únicos</b>{' '}
              (de {candidatePool.length} instancias / {new Set(candidatePool.map(s => s.tNorm)).size} tickers totales)
            </div>
          </Card>
          ...
          <Card className="p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <Button variant="outline" className="gap-1.5 text-[12px]" disabled={!candAnySelected || candExporting !== null} onClick={handleExportCandidatesHtml}>
                <FileCode size={14} /> {candExporting === 'html' ? '…' : 'HTML'}
              </Button>
              <Button className="gap-1.5 text-[12px] bg-foreground text-background hover:bg-foreground/90" disabled={!candAnySelected || candExporting !== null} onClick={handleExportCandidatesPdf}>
