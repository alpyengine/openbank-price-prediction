...
async function renderHtmlToPdf(html, filename) {
  ...
  // v7.22.7 — page margins (top/bottom/left/right), so nothing sits flush
  // against the paper edge (matters for printing — content that touches
  // the edge either gets clipped by the printer or just looks unfinished).
  const PAGE_MARGIN_MM = 15

  const pdf      = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  const pageW    = pdf.internal.pageSize.getWidth()
  const pageH    = pdf.internal.pageSize.getHeight()
  const contentW = pageW - PAGE_MARGIN_MM * 2   // usable width between left/right margins
  const usableH  = pageH - PAGE_MARGIN_MM * 2   // usable height between top/bottom margins
  ...
  // behaviour, now also margin-aware.
  if (blocks.length === 0) {
    ...
    while (yOffset < fullH) {
      ...
      pdf.addImage(imgData, 'PNG', PAGE_MARGIN_MM, PAGE_MARGIN_MM - yOffset, contentW, fullH)
      yOffset += usableH
    }
    pdf.save(filename)
  ...
  for (const block of blocks) {
    ...
    if (blockH > usableH) {
      ...
      while (yOffset < blockH) {
        ...
        pdf.addImage(imgData, 'PNG', PAGE_MARGIN_MM, PAGE_MARGIN_MM - yOffset, contentW, blockH)
        yOffset += usableH
        firstSlice = false
      }
    ...
    if (pageHasContent && cursorY + blockH > pageH - PAGE_MARGIN_MM) {
    ...
    pdf.addImage(imgData, 'PNG', PAGE_MARGIN_MM, cursorY, contentW, blockH)
